...
sensor_name=['超声波','墙检红外','预留','回充红外','地检红外']
...
def config_log_level():
    try:
        ...
        global str_ip
        if s and s=="Cruzr1S": str_ip="192.168.100.10"#
...
#初始化
        ...
        self.setWindowTitle('UBTECH-Robot V%0.1f '%VERSION)
        ...
        self.tabWidget_all.setTabText(self.tabWidget_all.indexOf(self.tab_4), "底部雷达")
        self.toolBox.setItemText(self.toolBox.indexOf(self.page_3), "预留")
...
#data val
        ...
        self.tabWidget_all.setTabEnabled(3, False)
...
#之前为了兼容Cruzr1添加显示，后面发现Cruzr1无法无法获取传感器数据暂保留
    ...
    def graph_sensor_init(self):
        ...
        try:
            ...
            # tof
            self.plt_disp_data[sum(sensor_index[0:2])+0] = self.plt.plot(pen="r", symbol='o', symbolPen='r', symbolSize=3, name="预留1")
            ...
            self.plt.legend.removeItem("预留1")
    ...
    def ros_topic_init(self,mode):
        ...
        try:
            if mode == "init" or (thread_class_scall.ros_client and not thread_class_scall.ros_client.is_connected):
                ...
                self.listener_untrasonic = Topic(thread_class_scall.ros_client, '/sensor_ultrasonic', 'u_msgs/SensorUltrasonic',throttle_rate=100,queue_size=1)
                ...
                self.listener_wall_check_ir = Topic(thread_class_scall.ros_client, '/sensor_wall_check_ir', 'u_msgs/SensorWallCheckIr',throttle_rate=100,queue_size=1)
                self.listener_charge_ir = Topic(thread_class_scall.ros_client, '/sensor_charge_ir', 'u_msgs/SensorChargeIr',throttle_rate=100,queue_size=1)
                ...
                self.listener_floor_ir = Topic(thread_class_scall.ros_client, '/sensor_floor_check_ir', 'u_msgs/SensorFloorCheckIr',throttle_rate=100,queue_size=1)
