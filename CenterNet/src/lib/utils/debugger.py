...
class Debugger(object):
  ...
  #donghang
  def show_distance(self,bboxx,show_txt=True,img_id='default'):
    bboxx = np.array(bboxx, dtype=np.int32)
    depth_frame = gol.get_value('aligned_depth_frame')
    text_depth = str(np.round(depth_frame.get_distance(int((bboxx[2]+bboxx[0])/2), int((bboxx[3]+bboxx[1])/2)),4))+"meter(s)" 
    #print(text_depth)
    cv2.circle(self.imgs[img_id],(int((bboxx[2]+bboxx[0])/2),int((bboxx[3]+bboxx[1])/2)),1,(0,255,255),1)
    cv2.putText(self.imgs[img_id], text_depth, (int((bboxx[2]+bboxx[0])/2),int((bboxx[3]+bboxx[1])/2)), cv2.FONT_HERSHEY_PLAIN, 1, (0,0,255), 1, cv2.LINE_AA)
  ...
  def show_all_imgs(self, pause=False, time=0):
    if not self.ipynb:
      ...
      if cv2.waitKey(1 if pause else 1) == 27:
        import sys
        sys.exit(0)
    else:
      self.ax = None
